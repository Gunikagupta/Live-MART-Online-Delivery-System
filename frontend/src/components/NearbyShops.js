...
const getNearbyItemsMock = async (lat, lng, maxDistance) => {
  return new Promise((resolve) => {
    setTimeout(() => {
      resolve({
        data: [
          { itemName: "Organic Brown Eggs", stockQuantity: 12, shopName: "Green Farm Organics", distanceKm: 1.2, price: 95 },
          { itemName: "Amul Butter (500g)", stockQuantity: 45, shopName: "SuperMart Daily", distanceKm: 0.8, price: 275 },
          { itemName: "Whole Wheat Bread", stockQuantity: 0, shopName: "The Bakery Junction", distanceKm: 2.5, price: 60 },
          { itemName: "Basmati Rice (5kg)", stockQuantity: 8, shopName: "Kirana King", distanceKm: 3.1, price: 850 },
          { itemName: "Fresh Paneer", stockQuantity: 15, shopName: "Daily Dairy", distanceKm: 1.5, price: 120 },
          { itemName: "Coca Cola (2L)", stockQuantity: 100, shopName: "Beverage Depot", distanceKm: 4.2, price: 90 },
        ]
      });
    }, 800);
  });
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div className="flex justify-between h-16 items-center">
        <div className="flex-shrink-0 flex items-center cursor-pointer">
          <Link to="/dashboard" className="text-2xl font-bold bg-gradient-to-r from-pink-900 via-red-700 to-pink-400 bg-clip-text text-transparent">
            GrocerGo
          </Link>
        </div>
        {showSearch && (
        ...
        <div className="flex items-center space-x-4">
          <Link to="/dashboard" className="text-gray-600 hover:text-pink-700 font-medium transition">Home</Link>
          <Link to="/cart" className="text-gray-600 hover:text-pink-700 font-medium transition">Cart</Link>
      ...
      <div className="py-12 px-6 max-w-7xl mx-auto">
        ...
        <div className="text-center mb-12">
          ...
          <div className="flex justify-center">
             {/* Updated to point to your Dashboard route */}
             <Link
                to="/dashboard" 
                className="px-8 py-3 text-lg font-semibold rounded-xl border border-pink-200 hover:border-pink-700 
                           bg-white shadow hover:shadow-lg transition text-pink-800 no-underline"
             >
                ← Back to Categories
             </Link>
          </div>
        ...
        <div className="bg-white rounded-3xl shadow-xl border border-pink-50 p-8 mb-12 relative overflow-hidden">
          ...
          <div className="flex flex-col sm:flex-row gap-5 relative z-10">
            ...
            {/* Updated to point to your Map route defined in App.js */}
            <button
            ...
            {nearbyItems.length === 0 ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {nearbyItems.map((item, index) => (
                  <div key={index} className="group bg-white rounded-3xl border border-pink-100 p-6 relative overflow-hidden shadow-sm hover:shadow-2xl hover:border-pink-300 transition-all duration-300 cursor-pointer">
